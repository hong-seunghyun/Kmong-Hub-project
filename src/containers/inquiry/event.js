...
import { getEventList } from "../../asset/apis/inquiry/eventApis";

const Component = () => {
  const [evnetList, setEvnetList] = useState([]);
  const [pageIdx, setPageIdx] = useState(1);

  const getValue = async () => {
    await getEventList({ currentIdx: pageIdx })
      .then((e) => setEvnetList([...e.data.data]))
      ...
      <div className="inquiry- ">
        <div className="transaction-01">
          <div className="head-wrap flex_">
            <h1 className="display-5-B">행사 문의 관리</h1>
          </div>
              ...
              {evnetList &&
                evnetList.map((e, idx) => (
                  <TableCell
                    ...
                    link={
                      e.prcsYn === "Y"
                        ? `/inquiry/event_detail_complete/${e.iqryNo}`
                        : `/inquiry/event_detail/${e.iqryNo}`
                    }
                  />
                ))}
              <Pagnation
