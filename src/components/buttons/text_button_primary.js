...
const Component = () => {
	...
	const TextButtonUnderlineL = () => {
		return(
		<button className={`btn_ txt-violet body-2-R ${isButton ? 'pressed' : ''}`} onClick={onClickActive} >
			<span className="flex_">
	...
	const TextButtonUnderlineXl = () => {
		return(
		<button className={`btn_ txt-violet body-1-R ${isButton ? 'pressed' : ''}`} onClick={onClickActive} >
			<span className="flex_">
			<Icon size={11.6} color='inherit' stroke='inherit' icon='plus' />
				label 
				<Icon size={13} color='inherit' stroke='inherit' icon='chevronRight' />
